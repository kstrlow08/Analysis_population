...
class EDA:
    ...
    def run_population_trends_eda(self):
        ...
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
            "기초 통계", "연도별 추이", "지역별 분석", "변화량 분석", "시각화", "심층 분석(추가)"
        ])
        ...
        with tab6:
            ...
            st.markdown("""
            #### 그래프 및 분석 해설
            - **출생아 수(Births)** **사망자 수(Deaths)** 추이를 통해 인구 변동의 핵심 요인을 파악할 수 있습니다.
            - **자연 증감(Natural Increase)** = `출생아 수 - 사망자 수`로, 양수(+)이면 인구가 자연적으로 증가하고, 음수(-)이면 자연적으로 감소함을 의미합니다.
